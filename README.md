...
The project includes focused coverage for:

- copy and move conflict handling
...
The Go application keeps archive operations, filesystem transactions, dialogs, progress handling, Trash support, and terminal rendering separated into focused source files rather than introducing a large framework around them.
...
- background job queue and job manager
- pause and resume for supported operations
- release packages for major Linux distributions
- broader automated terminal interaction tests
- optional localization
- native remote filesystem connections

These are roadmap ideas, not prerequisites for using the current commander.
...
Resize the terminal so both panels and the function-key bar fit comfortably.
